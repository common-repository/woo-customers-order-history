...
class Woo_Customers_Order_History_Admin {
	    ...
	    if ( ! freemius_wcoh()->can_use_premium_code()  ) {
	        if ( time() > $inst_unix_time && $wcchpo_message_date != date("Ymd") ) {
	            ...
	            if ( date( "D" ) == "Fri" ) {
	                ...
	                echo  "<p>Please, update <a href='" . esc_url(freemius_wcoh()->get_upgrade_url()) . "'> woocommerce customers order history to paid version. </a> </p>" ;
	                echo  "</div>" ;
	            }
	        }
	    }
	...
	*/
	public function wcchpo_render_meta_box_render( $post_info, $meta_box_info ){
        ...
        if ( !empty($total_number_of_orders) &&   $total_number_of_orders != 1 ) {
            ...
            echo  "<span class='Circle tips' style='background-color:" . esc_attr($total_colour) . "' data-tip='Total number of orders by this user'>" . esc_html($total_number_of_orders) . "</span>" ;
        }
        
        foreach ( $statuses_orders as $status_key => $status_number ) {
            if ( !empty($status_number) ) {
                echo  "<span class='Circle tips " . esc_attr($status_key) . "' style='background-color:" . $this->colour_func( $status_key, get_option( "wcchpo_colour_" . $status_key ) ) . "' data-tip='" . esc_attr($status_key) . "'>" . esc_html($status_number) . "</span>" ;
            }
        }
        # user note
        $wcchpo_user_note = get_user_meta( $customer_id, 'wcchpo_user_note', true );
        if ( !empty($wcchpo_user_note) ) {
            echo  "<span class='tips dashicons dashicons-admin-comments' style='padding:2px;' data-tip='" . esc_attr($wcchpo_user_note) . "' > </span>" ;
        }
	...
	*/
    public function colour_func( $value = '', $colour_options = '' ){
        ...
        if ( $value == 'pending' ) {
            $colour_code = ( empty($colour_options) ? "#AFFD71" : esc_attr($colour_options));
        } elseif ( $value == 'processing' ) {
            $colour_code = ( empty($colour_options) ? "#c6e1c6" : esc_attr($colour_options));
        } elseif ( $value == 'on-hold' ) {
            $colour_code = ( empty($colour_options) ? "#f8dda7" : esc_attr($colour_options));
        } elseif ( $value == 'completed' ) {
            $colour_code = ( empty($colour_options) ? "#61B329" : esc_attr($colour_options));
        } elseif ( $value == 'cancelled' ) {
            $colour_code = ( empty($colour_options) ? "#FF2400" : esc_attr($colour_options));
        } elseif ( $value == 'refunded' ) {
            $colour_code = ( empty($colour_options) ? "#092366" : esc_attr($colour_options));
        } elseif ( $value == 'failed' ) {
            $colour_code = ( empty($colour_options) ? "#CD0000" : esc_attr($colour_options));
        } else {
            $colour_code = ( empty($colour_options) ? "#000000" : esc_attr($colour_options));
        }
           ...
           # getting post meta 
            ...
            if ( $this->orderCacheStatus ){
                ...
                if ( ( isset($this->orderCacheData[ $customer_id ]["total"]) && in_array( $post_id, $this->orderCacheData[ $customer_id ]["total"]) )  ||  ( isset($this->orderCacheData[ $billing_email ]["total"]) && in_array($post_id, $this->orderCacheData[ $billing_email ]["total"]) ) ) {
                    ...
                    if ( isset($this->orderCacheData[ $customer_id ]) && is_array( $this->orderCacheData[$customer_id] ) ){
                        foreach( $this->orderCacheData[ $customer_id ] as $key => $value ) {
                            if ( count( $this->orderCacheData[ $customer_id]["total"] ) == 1   ){
                                if ( $key != "total" ){
                                    $outPut .= "<span class='Circle tips " . esc_attr($key) . "' style='background-color:" . esc_attr($this->colour_func( $key, get_option( 'wcchpo_colour_' . $key ))) . "' data-tip='" . esc_attr($key) . "'>" . esc_html(count( $value )) . "</span>" ; 
                                }
                            } else {
                                $outPut .= "<span class='Circle tips " . esc_attr($key) . "' style='background-color:" . esc_attr($this->colour_func( $key, get_option( 'wcchpo_colour_' . $key ))) . "' data-tip='" . esc_attr($key) . "'>" . esc_html(count( $value )) . "</span>" ;
                            }
                    ...
                    if ( $customer_id ) {
                        ...
                        foreach ( $this->orderCacheData[ $billing_email ] as $key => $value) {
                            ...
                            if ( count( $this->orderCacheData[ $billing_email ]["total"] ) == 1   ){
                                if ( $key != "total" ){
                                    $outPut .=  "<span class='Circle tips " . esc_attr($key) . "' style='background-color:" . esc_attr($this->colour_func( $key, get_option( 'wcchpo_colour_' . $key ) )) . "' data-tip='" . esc_attr($key) . "'>" . esc_html(count( $this->orderCacheData[$billing_email]["total"] )) . "</span>" ; 
                                }
                            } else {
                                $outPut .=  "<span class='Circle tips " . esc_attr($key) . "' style='background-color:" . esc_attr($this->colour_func( $key, get_option( 'wcchpo_colour_' . $key ) )) . "' data-tip='" . esc_attr($key) . "'>" . esc_html(count( $this->orderCacheData[$billing_email]["total"] )) . "</span>" ;
                            }
	            ...
	            if ( $this->displayUserNote  && !empty( $wcchpo_user_note ) ) {
	                $outPut .=  "<span class='tips dashicons dashicons-admin-comments' style='padding:2px;' data-tip='" . esc_attr($wcchpo_user_note) . "' > </span>" ;
                }
                # Display the History 
	            ...
	            foreach ( $statuses_orders as $status_key => $status_number ) {
	                if ( !empty( $status_number ) ) {
	                    echo  "<span class='Circle tips " . esc_attr($status_key) . "' style='background-color:" . esc_attr($this->colour_func( $status_key, get_option( 'wcchpo_colour_' . $status_key ) )) . "' data-tip='" . esc_attr($status_key) . "'>" . esc_html($status_number) . "</span>" ;
	                }
                }
	            ...
	            if (  $customer_id ) {
                    ...
                    if ( $total_number_of_orders > 1 ){
	                    ...
	                    'action' => 'order',
	                    'userID' => $customer_id,
	                    ), site_url( '/wp-admin/admin.php?page=wcchpo-history' ) ) ) . "' class='dashicons dashicons-external tips' style='cursor: alias; padding:2px;' data-order-id='" . esc_attr($post_id) . "' target='_blank' data-tip='This user orders history'>  </a>" ;
                    }
	            } else {
                    # check & balance is number of orders is not zero Do the task
                    if ( $total_number_of_orders > 1 ) {
    ...
    public function wcchpo_User_table_order_history_column_rows( $content, $column_name, $customer_id ){
        if ( $column_name == 'wcchpo_user_order_details' ) {               
            ...
            if ( ! empty($total_number_of_orders) ) {
                # getting the colours 
                $total_colour = ( empty( get_option( 'wcchpo_colour_total' ) ) ? "#cccccc" : get_option( 'wcchpo_colour_total' ) );
                $content .= "<span class='Circle tips' style='background-color: " . esc_attr($total_colour) . "; cursor: default;' title='Total number of orders by this user'>" . esc_html($total_number_of_orders) . "</span>";
                # Looping the Order statuses
                foreach ( $statuses_orders as $status_key => $status_number ) {
                    if ( ! empty( $status_number ) ) {
                        $content .= "<span class='Circle tips " . esc_attr($status_key) . "' style='background-color:" . esc_attr($this->colour_func( $status_key, get_option( 'wcchpo_colour_' . $status_key ) )) . "; cursor: default;' title='" . esc_attr($status_key) . "'>" . esc_html($status_number) . "</span>";
                    }
                }
                ...
                $wcchpo_user_note = get_user_meta( $customer_id, 'wcchpo_user_note', true );
                if ( ! empty( $wcchpo_user_note ) ) {
                    $content .= "<span class='dashicons dashicons-admin-comments' style='padding:2px;' title='" . esc_attr($wcchpo_user_note) . "' ></span>";
                }
    ...
    public function wcchpo_useragent( $user_agent_txt = '' ){
        ...
        if ( strpos( $user_agent_txt, 'Windows' ) !== false ) {
            ...
            if ( strpos( $user_agent_txt, 'Mobile' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> Windows Mobile   </span>";
            } elseif ( strpos( $user_agent_txt, 'Phone' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> Windows phone    </span>";
            } else {
                return "<span title='".esc_attr($user_agent_txt)."'> ¯\_(ツ)_/¯        </span>";
            }
            
            return "<span title='".esc_attr($user_agent_txt)."'> Windows";
        } elseif ( strpos( $user_agent_txt, 'Linux' ) !== false ) {
            // Linux
            if ( strpos( $user_agent_txt, 'Android' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> Android  </span>";
            } elseif ( strpos( $user_agent_txt, 'Ubuntu' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> Ubuntu   </span>";
            } elseif ( strpos( $user_agent_txt, 'NetCast' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> NetCast  </span>";
            } elseif ( strpos( $user_agent_txt, 'SMART-TV' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> SMART-TV </span>";
            } elseif ( strpos( $user_agent_txt, 'X11' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> Linux    </span>";
            } elseif ( strpos( $user_agent_txt, 'Web0S' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> Web0S    </span>";
            } else {
                return "<span title='".esc_attr($user_agent_txt)."'> ¯\_(ツ)_/¯</span>";
            }
            ...
            if ( strpos( $user_agent_txt, 'iPhone' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> iPhone   </span>";
            } elseif ( strpos( $user_agent_txt, 'iPad' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> iPad     </span>";
            } elseif ( strpos( $user_agent_txt, 'Macintosh' ) !== false ) {
                return "<span title='".esc_attr($user_agent_txt)."'> Macintosh </span>";
            } else {
                return "<span  title='".esc_attr($user_agent_txt)."'>¯\_(ツ)_/¯ </span>";
            }
            
            return "<span title='".esc_attr($user_agent_txt)."'> Mac os    </span>";
        } elseif ( strpos( $user_agent_txt, 'Nokia' ) !== false ) {
            // Symbion
            return "<span title='".esc_attr($user_agent_txt)."'> Symbian   </span>";
        } else {
            return "<span title='".esc_attr($user_agent_txt)."'> ¯\_(ツ)_/¯ </span>";
        }
    }
