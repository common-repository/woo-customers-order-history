	...
	<div id="poststuff">
		...
		<?php
			foreach ($current_statuses_orders as $order_status_arr_name => $order_status_arr ) {
				...
				echo "<p><b style='padding:3px; text-transform: capitalize;'>".esc_html($order_status_arr_name) .":</b> </p>";
				?>
					<table class="widefat">
						<thead>
							<tr>
								...
								foreach ($order_status_arr as  $order_id) {
									...
									foreach ($items as $item) {
										echo (++$i % 2) ? "<tr class='alternate'>" : "<tr>";
											echo "<td>". esc_html($an) ."</td>";
											echo "<td><a href='". esc_url( add_query_arg( array('post'=>$orders[$order_id]->get_id() ,'action'=>'edit') , site_url('/wp-admin/post.php') ) )  ."' >". esc_html($orders[$order_id]->get_id()) ." </a></td>";
											$url = esc_url(get_the_permalink( $item['product_id']));
											echo "<td> <a href='".esc_attr($url)."'> ". esc_html($item['product_id'])  ."</a></td>";
											echo "<td> <a href='".esc_attr($url)."'> ". esc_html($item['name']) ."</a></td>";
											echo "<td>";
											echo esc_html($item['quantity']);
											if ( isset( $rf[ $orders[$order_id]->get_id() ] , $rf[ $orders[$order_id]->get_id() ][$item['product_id']] )  ){
												echo "  <code> <span style='color: red;' >" . esc_html(abs( $rf[ $orders[$order_id]->get_id() ][$item['product_id']] )) . "</span> Refunded </code> ";
											}
											echo "</td>";
											echo "<td>". esc_html($item['total']) ."</td>";
										echo "</tr>";
										$an ++;
									}
								}
