<?php 
	...
	if ( isset(  $_GET['userID'] ) ) {
		...
		$userName = esc_html($_GET['userEmail']);
	} 

	# CSV Starts From Here 
	header('Content-Type: text/csv');
	header('Content-Disposition: attachment; filename="'. esc_attr(date(get_option('date_format'))) .'_'.$userName.'_orders.csv"');
	...
	foreach ( $orders as $order => $order_data) {
		...
		fputcsv($file, 
			array(
				...
				$order_data->get_billing_city(). " - " . esc_html($order_data->get_billing_postcode()) .", ". esc_html($order_data->get_billing_country()) ,
